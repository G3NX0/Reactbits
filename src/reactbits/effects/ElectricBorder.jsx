...
export default function ElectricBorder({
  ...
  const strokeCoreRef = useRef(null);
  ...
  const updateAnim = () => {
    ...
    [strokeColorRef.current, strokeCoreRef.current].forEach((el) => {
      if (el) el.style.filter = `url(#${filterId})`;
    });
    ...
    const esc = (s) => {
      try { return (CSS && typeof CSS.escape === 'function') ? CSS.escape(s) : s; } catch { return s; }
    };
    const filterEl = svg.querySelector(`#${esc(filterId)}`);
    if (filterEl) {
  ...
  return (
    <div ref={rootRef} className={`electric-border ${className}`} style={{ ...vars, ...style }}>
      ...
      <div className="eb-layers">
        <div ref={strokeColorRef} className="eb-stroke eb-stroke--color" />
        <div ref={strokeCoreRef} className="eb-stroke eb-stroke--core" />
